...
import ShadeTool_Manager from '../Shade/ShadeTool_Manager'
import ShaderTool_Algorithm from '../Shade/ShadeTool_Algorithm'

import useShadeStore, { MULTI_SOURCE_COLORS, buildSourcesList } from './store'
import ShadePanel from './components/ShadePanel'

import './ShadeToolNew.css'
...
let ShadeToolNew = {
    ...
    initialize: function () {
        const vars = L_.getToolVars('shadenew')
        useShadeStore.getState().setVars(vars)

        if (vars && vars.__noVars !== true) {
            if (vars.data == null)
                console.warn(
                    'ShadeToolNew: variables object does not contain key "data"!'
                )
            else if (vars.data.length == null)
                console.warn(
                    'ShadeToolNew: variables object "data" is not an array!'
                )
            else if (vars.data.length == 0)
                console.warn('ShadeToolNew: variables object "data" is empty!')
        }
    },
    ...
    make: function () {
        ...
        const rawTime = ShadeToolNew.parseToUTCTime(TimeControl.getEndTime())
        store.setSweepField('rawTime', rawTime)
        store.setSweepField(
            'utcTime',
            ShadeToolNew.parseToUTCTime(TimeControl.getEndTime(), true)
        )
        ...
        if (!ShadeToolNew._root) {
            ShadeToolNew._root = createRoot(
                document.getElementById('tools')
            )
        }
        ShadeToolNew._root.render(<ShadePanel />)

        Map_.map.on('click', ShadeToolNew._onMapClick)
        Map_.map.on('moveend', ShadeToolNew._onPanEnd)
        TimeControl.subscribe('ShadeToolNew', (t) => {
            const raw = ShadeToolNew.parseToUTCTime(t.currentTime)
            useShadeStore.getState().setSweepField('rawTime', raw)
            useShadeStore.getState().setSweepField(
                'utcTime',
                ShadeToolNew.parseToUTCTime(t.currentTime, true)
            )
            ShadeToolNew._onTimeChange(raw)
        })
    },

    destroy: function () {
        if (ShadeToolNew._sweepPlayTimer) {
            clearInterval(ShadeToolNew._sweepPlayTimer)
            ShadeToolNew._sweepPlayTimer = null
        }
        Map_.map.off('click', ShadeToolNew._onMapClick)
        Map_.map.off('moveend', ShadeToolNew._onPanEnd)
        TimeControl.unsubscribe('ShadeToolNew')

        if (ShadeToolNew._root) {
            ShadeToolNew._root.unmount()
            ShadeToolNew._root = null
        }
    ...
    _onPanEnd: function () {
        ...
        if (el && el.resolution <= store.vars?.dynamicUpdateResCutoff) {
            ShadeToolNew.shade(null, store.activeElmId)
        }
    },
    ...
    shade: function (source, activeElmId, ignoreMarker, initObj) {
        ...
        calls.api(
            ...
            function (data) {
                if (data[0] && data[0][1] != null) {
                    ...
                    Promise.all(ll2aerllPromises).then((results) => {
                        ShadeToolNew.updateRAEIndicators(
                            results[0],
                            activeElmId,
                            results
                        )
            ...
            function () {
                console.warn(
                    'ShadeToolNew: Failed to query center elevation.'
                )
        ...
        function keepGoing(targetSources) {
            ...
            ShadeTool_Manager.gatherTiles(
                ...
                function (data) {
                    ...
                    ShadeToolNew.renderResultToMap(
                        data,
                        compositedResult,
                        options,
                        activeElmId
    ...
    renderResultToMap: function (data, resultGrid, options, activeElmId) {
        ...
        ShadeToolNew.makeDataLayer(dl, activeElmId)
    },
                    ...
                    function (data) {
                        ...
                        function processBatch(batchStart) {
                            ...
                            for (let ti = batchStart; ti < batchEnd; ti++) {
                                ...
                                const timeStr =
                                    ShadeToolNew.parseToUTCTime(ts)
                            ...
                            Promise.all(batchPromises).then((batchResults) => {
                                ...
                                if (completed >= total) {
                                    ...
                                    if (sweepGrids.length > 0) {
                                        ...
                                        ShadeToolNew.renderHeatmapToMap(
                                            data,
                                            heatmap,
                                            activeElmId
                                        )
    ...
    sweepStepForward: function () {
        ...
        ShadeToolNew.sweepShowFrame(store.activeElmId)
    },
    ...
    sweepShowFrame: function (activeElmId) {
        ...
        ShadeToolNew.renderResultToMap(data, grid, options, activeElmId)
    ...
    updateSweepSpeed: function (speed) {
        const store = useShadeStore.getState()
        if (store.sweepPlaying && ShadeToolNew._sweepPlayTimer) {
            clearInterval(ShadeToolNew._sweepPlayTimer)
            ShadeToolNew._sweepPlayTimer = setInterval(function () {
                const s = useShadeStore.getState()
                const nextIdx =
    ...
    updateRAEIndicators(rae, shadeId, allResults) {
        ...
        if (rae.error != true) {
            ...
            if (rae.ancillary?.sun_az) {
                ...
                ShadeToolNew.drawAzAngleGuideOnCanvas(
                    ctxAz,
                    origin,
                    sizeInner,
        ...
        if (rae.error != true) {
            if (rae.ancillary?.sun_el) {
                ShadeToolNew.drawElAngleGuideOnCanvas(
                    ctxEl,
                    origin,
                    sizeInner,
