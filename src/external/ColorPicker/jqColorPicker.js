;(function (root, factory) {
    if (typeof exports === 'object') {
        module.exports = factory(window, require('jquery'), require('./colors'))
    ...
    var $document = $(document),
        ...
        _html =
            '<div class="cp-color-picker"><div class="cp-z-slider"><div c' +
        ...
        _css =
            '.cp-color-picker{z-index:3000;position:absolute;overflow:hidden;padding:6p' +
    ...
    function toggle(event) {
        ...
        if (event) {
            ...
            $(_$UI)
                .hide(_options.animationSpeed, function () {
                    preRender(false)
        ...
        return $(_html)
            ...
            .show(0, function () {
                ...
                _$UI.prepend('<div>')
                    .children()
                    .eq(0)
                    .css('width', _$UI.children().eq(0).width()) // stabilizer
    ...
    function preRender(toggled) {
        var colors = _color.colors,
            ...
            colorText = _color.toString(
                _$trigger._colorMode,
                _options.forceAlpha
            ),
            ...
            hasNoValue =
                _$trigger[0].hasAttribute('value') && // question this
                triggerValue === '' &&
                toggled !== undefined
        ...
        _$trigger._css = {
            ...
            color: hasNoValue
                ? ''
                : colors.rgbaMixBGMixCustom.luminance > 0.22
                ? dark
                : light,
        }
        _$trigger.text = hasNoValue
            ? ''
            : triggerValue !== colorText
            ? colorText
            : ''
    ...
    $.fn.colorPicker = function (options) {
        ...
        !_colorPicker &&
            options.scrollResize &&
            $(window).on('resize.tcp scroll.tcp', function () {
        ...
        $(options.body)
            ...
            .on(_pointerdown, function (e) {
                _instance
                    .add(_$UI)
                    .add($(_$UI).find(e.target))
                    .index(e.target) === -1 && toggle()
            })

        return this.on('focusin.tcp click.tcp', function (event) {
            _colorPicker.color.options = $.extend(
                // swap options to fake new instance
            ...
            .each(function () {
                ...
                $elm.data(
                    'colorMode',
                    mode[1] ? mode[0].substr(0, 3) : 'HEX'
                ).attr('readonly', _options.preventFocus)
                options.doRender &&
                    $elm.css({
                        'background-color': value,
                        color: function () {
                            return _color.setColor(value).rgbaMixBGMixCustom
                                .luminance > 0.22
                                ? options.dark
                                : options.light
                        },
