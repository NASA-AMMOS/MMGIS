;(function (root, factory) {
    if (typeof exports === 'object') {
        module.exports = factory(window, root)
    ...
    var _valueRanges = {
        ...
        initInstance = function (THIS, options) {
            ...
            _options.customBG =
                typeof customBG === 'string'
                    ? ColorConverter.txt2color(customBG).rgb
                    : customBG
    ...
    Colors.prototype.setCustomBackground = function (col) {
        ...
        this.options.customBG =
            typeof col === 'string' ? ColorConverter.txt2color(col).rgb : col
    ...
    Colors.prototype.toString = function (colorMode, forceAlpha) {
        return ColorConverter.color2text(
            (colorMode || 'rgb').toLowerCase(),
            this.colors,
            forceAlpha
        )
    }
    ...
    function setColor(colors, color, type, save, alpha) {
        ...
        if (typeof color === 'string') {
            ...
            for (var n in color) {
                colors[type][n] = limitValue(
                    color[n] / _valueRanges[type][n][1],
                    0,
                    1
                )
            }
    ...
    function saveAsBackground(RGB, rgb, alpha) {
        ...
        color.equivalentGrey = _round(
            grey.r * RGB.r + grey.g * RGB.g + grey.b * RGB.b
        )
        ...
        if (_instance.options.customBG) {
            color.rgbaMixCustom = mixColors(
                rgb,
                _instance.options.customBG,
                alpha,
                1
            )
            color.rgbaMixCustom.luminance = getLuminance(
                color.rgbaMixCustom,
                true
            )
            if (typeof _instance.options.customBG !== 'string')
                _instance.options.customBG.luminance = getLuminance(
                    _instance.options.customBG,
                    true
                )
        }
    ...
    function convertColors(type, colorObj) {
        ...
        if (type !== 'alpha') {
            ...
            colors.saveColor =
                RGB.r === SAVE.r && RGB.g === SAVE.g && RGB.b === SAVE.b
                    ? 'web save'
                    : RGB.r === SMART.r &&
                      RGB.g === SMART.g &&
                      RGB.b === SMART.b
            ...
            if (colorObj) {
                colors.background = saveAsBackground(
                    RGB,
                    colors.rgb,
                    colors.alpha
                )
        ...
        if (options.customBG) {
            rgbaMixBGMixCustom = mixColors(
                rgb,
                background.rgbaMixCustom,
                alpha,
                1
            )
            rgbaMixBGMixCustom[luminance] = getLuminance(
                rgbaMixBGMixCustom,
                true
            )
            rgbaMixBGMixCustom.WCAG2Ratio = getWCAG2Ratio(
                rgbaMixBGMixCustom[luminance],
            ...
            rgbaMixBGMixCustom.hueDelta = getHueDelta(
                background.rgbaMixCustom,
                rgbaMixBGMixCustom,
                true
            )
    ...
    var ColorConverter = {
        ...
        color2text: function (colorMode, colors, forceAlpha) {
            var alpha =
                    forceAlpha !== false && _round(colors.alpha * 100) / 100,
                hasAlpha =
                    typeof alpha === 'number' &&
                    forceAlpha !== false &&
                    (forceAlpha || alpha !== 1),
        ...
        rgb2hsv: function (rgb) {
            ...
            return {
                ...
                s: r
                    ? chroma / r
                    : (_colors && _colors.hsv && _colors.hsv.s) || 0, // ??_colors.hsv.s || 0
                v: r,
            }
        },
        ...
        hsv2hsl: function (hsv) {
            ...
            return {
                ...
                s:
                    !hsv.v && !s
                        ? (_colors && _colors.hsl && _colors.hsl.s) || 0
                        : s, // ???
                l: l / 2,
            }
        },
    ...
    function getHueDelta(rgb1, rgb2, nominal) {
        return (
            ((_Math.max(rgb1.r - rgb2.r, rgb2.r - rgb1.r) +
                _Math.max(rgb1.g - rgb2.g, rgb2.g - rgb1.g) +
                _Math.max(rgb1.b - rgb2.b, rgb2.b - rgb1.b)) *
                (nominal ? 255 : 1)) /
            765
        )
    }
    ...
    function getLuminance(rgb, normalized) {
        ...
        for (var i = RGB.length; i--; ) {
            RGB[i] =
                RGB[i] <= 0.03928
                    ? RGB[i] / 12.92
                    : _Math.pow((RGB[i] + 0.055) / 1.055, 2.4)
        }
        return (
