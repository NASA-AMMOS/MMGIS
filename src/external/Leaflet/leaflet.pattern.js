...
;(function (window, document, undefined) {
    ...
    /*
     * L.ImagePattern is an implementation of Pattern that creates patterns from images.
     */

    L.ImagePattern = L.Pattern.extend({
        options: {
            url: '',
            width: 128,
            height: 128,
        ...
        _addShapes: function () {
            this._image = new L.PatternImage({
    ...
    L.simagePattern = function (options) {
        return new L.ImagePattern(options)
    }
        ...
        _updateStyle: function (options) {
            ...
            if (options.fill) {
                if (options.fillPattern) {
                    dom.setAttribute(
                        'style',
                        'fill: url(#' + L.stamp(options.fillPattern) + ')'
                    )
                } else {
    ...
    /*
     * L.PatternImage is the implementation of PatternImages for adding Images
     */

    L.PatternImage = L.PatternShape.extend({
        options: {
            image: true,
        },
        _initDom: function () {
