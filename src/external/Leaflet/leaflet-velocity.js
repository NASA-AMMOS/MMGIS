...
L.VelocityLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    setOpacity: function setOpacity(opacity) {
        if (this._canvasLayer?._canvas) {
            if (opacity > 0) this._lastNonZeroOpacity = opacity
    ...
    onDrawLayer: function onDrawLayer(overlay, params) {
        ...
        if (this._timer2) clearTimeout(self._timer2)
    ...
    _startWindy: function _startWindy() {
        ...
        this._timer2 = setTimeout(() => {
            this.setOpacity(this._lastNonZeroOpacity || 1)
        }, 350)
    },
...
var Windy = function Windy(params) {
    ...
    var buildGrid = function buildGrid(data, callback) {
        ...
        if (!supported)
            console.log(
                'Windy Error: data must have at least two components (u,v)'
            )
        ...
        if ('lo1' in header == false) {
            ...
            header = {
                numberPoints: data.features.length,
                lo1: -180,
                lo2: 180,
                la1: 90,
                la2: -90,
            ...
            colorScale.indexFor = function (m) {
                ...
                return Math.max(
                    ...
                    Math.min(
                        colorScale.length - 1,
                        Math.round(
                            ((m - min) / (max - min)) * (colorScale.length - 1)
                        )
                    )
                )
            }
