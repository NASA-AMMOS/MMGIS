...
# python create-stac-items.py [mmgis_url] [mmgis_token] [collection_id] [file_or_folder_path] [--options]
# ex: python create-stac-items.py "http://localhost:8888" stac-token myCollection C:\Path\to\COGs --upsert --path_remove /var/www/html/mmgis/MMGIS/ --path_replace_with /
# ex: python create-stac-items.py "http://localhost:8888" stac-token myCollection C:\Path\to\COGs --output-file output.json --path_remove /local/path/ --path_replace_with /production/path/
