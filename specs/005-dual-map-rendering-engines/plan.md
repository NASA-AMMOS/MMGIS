...
## 13. Viewer Panel Implementation ⭐ NEW

### 13.1 Architecture

**File Structure**:
```
src/essence/Basics/Viewer_/
├── Viewer_.js                    # Main controller module
├── OpenSeadragonViewer.js        # DZI image viewer
├── PhotosphereViewer.js          # 360° panorama viewer
├── ModelViewer.js                # 3D model viewer
├── PDFViewer.js                  # PDF document viewer
├── VideoPlayer.js                # Video playback
├── ImageAdjustments.js           # Brightness/contrast/saturation
└── ViewerAnnotations.js          # Image annotation system
```

### 13.2 Viewer Controller
...
const Viewer_ = {
    ...
    init: function() {
        ...
        this._setupModeIndicator()
    },

    open: function() {
        this.container.classList.add('visible')
        CssLoader.updateSize()  // Resize panels
    },

    close: function() {
        this.container.classList.remove('visible')
        this._disposeCurrentViewer()
        CssLoader.updateSize()
    },
    ...
    loadVideo: function(url, type = 'video/mp4') {
        ...
        this.videoPlayer.loadVideo(url, type)
    ...
    _updateModeIndicator: function(mode) {
        const indicator = document.getElementById('viewer-mode-indicator')
        const labels = {
            image: 'Image Viewer',
            photosphere: '360° Photosphere',
            model: '3D Model',
            pdf: 'PDF Document',
            video: 'Video'
        }
...
### 13.3 OpenSeadragon Image Viewer

**Purpose**: Deep zoom tiled imagery viewer for gigapixel images
...
class OpenSeadragonViewer {
    ...
    _init() {
        // Clear container
        this.container.innerHTML = '<div id="openseadragon-container"></div>'

        // Initialize OpenSeadragon
        this.viewer = OpenSeadragon({
            element: document.getElementById('openseadragon-container'),
        ...
        this.adjustments = new ImageAdjustments(this.viewer)

        // Setup annotations (if enabled)
        this.annotations = new ViewerAnnotations(this.viewer)
    }

    open(source) {
        if (typeof source === 'string') {
            // URL to DZI or simple image
            ...
            // Tiled image source configuration
            this.viewer.open(source)
        }
    }
...
**DZI Tile Source Configuration**:
```javascript
// For pre-generated DZI
viewer.open('/path/to/image.dzi')
...
viewer.open({
    ...
    height: 10200,
    tileSize: 256,
    tilesUrl: '/path/to/TileGroup{g}/{z}-{x}-{y}.jpg'
})

// For IIIF Image API
viewer.open({
    type: 'image',
    url: 'https://iiif.example.com/image/id/info.json'
})
```

### 13.4 Image Adjustments
...
class ImageAdjustments {
    ...
    _createControls() {
        const controlsHtml = `
            <div id="image-adjustment-controls" class="viewer-controls">
                ...
                <div class="control-group">
                    ...
                    <button id="rotate-reset" title="Reset rotation">⟲</button>
                    <button id="rotate-right" title="Rotate 90° right">↷</button>
                </div>
                <button id="reset-all" class="reset-button">Reset All</button>
            </div>
        `

        const container = document.createElement('div')
        container.innerHTML = controlsHtml
        document.getElementById('viewer-sidebar').appendChild(container)
    ...
    reset() {
        ...
        $('#brightness-value').text('100%')
        $('#contrast-value').text('100%')
        $('#saturation-value').text('100%')
    ...
    dispose() {
        $('#image-adjustment-controls').remove()
    }
}
```

### 13.5 Photosphere Viewer
...
class PhotosphereViewer {
    ...
    _init() {
        // Clear container
        this.container.innerHTML = '<div id="photosphere-container"></div>'
        const container = document.getElementById('photosphere-container')
        ...
        this.controls.rotateSpeed = -0.5  // Invert for natural inside-sphere feel
        this.controls.minDistance = 0.1
        this.controls.maxDistance = 100

        // Setup FOV controls
        this._setupFOVControls()
    ...
    _createSphere() {
        // Create inverted sphere for inside view
        const geometry = new THREE.SphereGeometry(
            500,  // radius
            60,   // width segments
            40    // height segments
        )
        geometry.scale(-1, 1, 1)  // Invert X for inside view
    ...
    loadImage(imageUrl) {
        ...
        textureLoader.load(
            ...
            undefined,
            (error) => {
                console.error('Error loading photosphere:', error)
            }
        )
    }
    ...
    _setupFOVControls() {
        const html = `
            <div id="photosphere-fov-controls">
                <label>Field of View</label>
                <input type="range" id="fov-slider"
                       min="30" max="120" value="75" step="5">
        ...
        document.getElementById('viewer-sidebar').appendChild(controls)

        $('#fov-slider').on('input', (e) => {
            this.setFOV(parseInt(e.target.value))
            $('#fov-value').text(e.target.value + '°')
        })
    }

    setFOV(degrees) {
        this.camera.fov = Math.max(30, Math.min(120, degrees))
    ...
    dispose() {
        ...
        $('#photosphere-fov-controls').remove()
    }
}
```
...
class ModelViewer {
    ...
    _init() {
        // Clear container
        this.container.innerHTML = '<div id="model-viewer-container"></div>'
    ...
    loadOBJ(objUrl, mtlUrl) {
        ...
        mtlLoader.load(mtlUrl, (materials) => {
            ...
            this._loadOBJWithoutMTL(objUrl)
        })
    }

    _loadOBJWithoutMTL(objUrl) {
        const objLoader = new THREE.OBJLoader()
        objLoader.load(objUrl, (object) => {
    ...
    _fitCameraToModel() {
        ...
        this.camera.position.set(cameraZ, cameraZ * 0.5, cameraZ * 2)
...
class PDFViewer {
        ...
        this.container.innerHTML = `
            <div id="pdf-viewer-container">
                <div class="pdf-controls">
                    <button id="pdf-first" title="First page">⏮</button>
    ...
    async _renderPage(num) {
        ...
        try {
            ...
            // Render pending page if requested during render
    ...
    _updatePageInfo() {
        $('#pdf-page-info').text(`Page ${this.pageNum} of ${this.pageCount}`)
    }

    _updateZoomInfo() {
        $('#pdf-zoom-info').text(`${Math.round(this.scale * 100)}%`)
    }
...
class VideoPlayer {
    ...
    _init() {
        ...
        this.container.innerHTML = `
            <div id="video-player-container">
                <video id="viewer-video" preload="metadata">
                    <source src="" type="">
                    Your browser does not support the video tag.
                </video>
                <div class="video-controls">
                    <button id="video-play-pause" class="control-button">
                        <span class="icon-play">▶</span>
                    </button>
                    <button id="video-stop" class="control-button" title="Stop">
                        <span>◼</span>
                    </button>
                    <input type="range" id="video-seek" class="seek-slider"
                           value="0" min="0" max="100" step="0.1">
                    <span id="video-time" class="time-display">0:00 / 0:00</span>
                    <button id="video-mute" class="control-button" title="Mute">
                        <span class="icon-volume">🔊</span>
                    </button>
                    <input type="range" id="video-volume" class="volume-slider"
                           value="100" min="0" max="100" step="1">
                    <button id="video-fullscreen" class="control-button" title="Fullscreen">
                        <span>⛶</span>
                    </button>
                </div>
            </div>
    ...
    _bindControls() {
        const playPauseBtn = $('#video-play-pause')
        const stopBtn = $('#video-stop')
        const seekSlider = $('#video-seek')
        const timeDisplay = $('#video-time')
        const muteBtn = $('#video-mute')
        const volumeSlider = $('#video-volume')
        const fullscreenBtn = $('#video-fullscreen')

        // Play/Pause
        playPauseBtn.on('click', () => {
            if (this.video.paused) {
                this.video.play()
                playPauseBtn.find('.icon-play').text('⏸')
            } else {
                this.video.pause()
                playPauseBtn.find('.icon-play').text('▶')
            }
        ...
        muteBtn.on('click', () => {
            ...
            muteBtn.find('.icon-volume').text(this.video.muted ? '🔇' : '🔊')
        })
        ...
        $(document).on('keydown.videoplayer', (e) => {
            if (!Viewer_.isOpen() || Viewer_.currentMode !== 'video') return
            ...
            switch (e.key) {
                ...
                case 'ArrowUp':
                    ...
                    volumeSlider.val(this.video.volume * 100)
                    break
                case 'ArrowDown':
                    e.preventDefault()
...
```json
{
  ...
  "properties": {
    ...
    "viewer": {
      "type": "image",
      "url": "Missions/Mars/Images/site_highres.dzi"
    }
  },
  "geometry": {
    "type": "Point",
...
```json
{
  "properties": {
    ...
    "viewers": [
      {
        "type": "photosphere",
        "url": "Images/site_alpha_360.jpg",
        "label": "360° Panorama"
      },
      {
        "type": "model",
        "url": "Models/site_alpha_model.gltf",
        "label": "3D Model"
      },
      {
        "type": "pdf",
        "url": "Documents/site_alpha_report.pdf",
        "label": "Site Report"
      }
    ]
  }
...
**Viewer Selection UI**:
```javascript
// When multiple viewers available
if (feature.properties.viewers && feature.properties.viewers.length > 1) {
    // Show viewer selection dropdown
    const menu = `
        <div class="viewer-selection-menu">
            ${feature.properties.viewers.map((v, i) => `
                <button data-index="${i}" class="viewer-option">
                    ${v.label || v.type}
                </button>
            `).join('')}
        </div>
    ...
    // User selects viewer
    $('.viewer-option').on('click', function() {
        const index = $(this).data('index')
        const viewerConfig = feature.properties.viewers[index]
        Viewer_.loadContent(viewerConfig)
    })
} else {
    // Single viewer - auto-open
    const viewerConfig = feature.properties.viewer || feature.properties.viewers[0]
    Viewer_.loadContent(viewerConfig)
}
```
