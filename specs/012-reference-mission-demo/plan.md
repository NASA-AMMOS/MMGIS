# Reference Mission Demo Mission - Technical Plan
...
### Reference Mission Maintenance Requirement
**Compliance**: ✅ (Self-fulfilling)
**Notes**:
- This feature *creates* the Reference Mission mission
- Constitution updated to require future features update Reference Mission
...
```
Reference Mission Mission Structure:

Missions/
└── Reference-Mission/
    ├── config.reference-mission.json         # Main configuration file
...
**Component 1: Mission Configuration (config.reference-mission.json)**
...
- **Responsibilities**:
  - Define mission metadata (name: "Reference-Mission", site, view, projection)
...
- **Responsibilities**:
  ...
  - **Optional local tiles**: Placeholder URLs with TODOs
    - Config includes: `"url": "Missions/Reference-Mission/Layers/Tiles/basemap/{z}/{x}/{y}.png"`
    - README documents: "Optional: Add your own tiles here"
  - **Optional GeoTIFF/COG**: Placeholder URLs with TODOs
    - Config includes: `"url": "Missions/Reference-Mission/Data/elevation.tif"`
    - README documents: "Optional: Add DEM for elevation profiles"
  - **Optional DEM/slope/cost data**: Placeholder URLs with TODOs
    - Measure Tool: `"dem": "Missions/Reference-Mission/Data/elevation.tif"`
    - Viewshed Tool: `"demtileurl": "Missions/Reference-Mission/Layers/DEMTiles/{z}/{x}/{y}.png"`
...
- **Purpose**: Comprehensive usage guide for Reference Mission mission
- **Responsibilities**:
  - **Overview**: Purpose of Reference Mission (reference, demo, testing)
...
**Component 7: Playwright Test Suite (tests/e2e/reference-mission.spec.js)**
- **Purpose**: Smoke tests validating Reference Mission configuration
- **Responsibilities**:
...
- **Interfaces**:
  - Single test file: `tests/e2e/reference-mission.spec.js`
  - Playwright test format
  - Runs in CI/CD pipeline
  - Sets `FORCE_CONFIG_PATH=Missions/Reference-Mission/config.reference-mission.json`
...
```
Initialization Flow:
  1. User sets FORCE_CONFIG_PATH=Missions/Reference-Mission/config.reference-mission.json
  2. User starts MMGIS: npm start
  3. Backend init-db.js runs (standard setup, no Reference Mission-specific logic)
  4. Backend server starts

Frontend Load Flow:
  5. Frontend detects FORCE_CONFIG_PATH in environment
  6. **TODO: Verify behavior** - Frontend loads config.reference-mission.json
...
Layer Rendering:
  ...
  13. For each tile layer (local, optional):
      ...
      - Request tiles from Missions/Reference-Mission/Layers/Tiles/
...
**Rationale**:
- Reference Mission is a configuration-only feature
...
**No New APIs**: Reference Mission uses existing MMGIS APIs.
...
**GET /Missions/Reference-Mission/Layers/Vectors/\*.geojson**
- **Purpose**: Serve vector GeoJSON files
- **Behavior**: Static file serving via Express
- **No changes required**

**GET /Missions/Reference-Mission/Data/\*.tif**
...
- May need separate Mars/Moon Reference Mission configs later for planetary use cases
...
- **Optional local raster**: Placeholder URLs with TODOs
  - Config: `"url": "Missions/Reference-Mission/Data/elevation.tif"`
...
- **Simplicity**: One `config.reference-mission.json` easier to understand and maintain
- **Focus**: Earth, Web Mercator, Leaflet/Cesium covers most use cases
- **Easy Refactoring**: When Mars/Moon configs actually needed:
  - Rename `config.reference-mission.json` → `config.reference-mission-earth.json`
  - Create `config.reference-mission-mars.json`
...
**Phase 1 Scope**:
- Create: `config.reference-mission.json` (Earth, Web Mercator, standard tools)
- Document: This is the default/only config
- Test: Single test file `reference-mission.spec.js`

**Future** (when actually needed):
- Refactor to `config.reference-mission-{name}.json` pattern
...
**Decision**: Multi-Config Pattern (`config.reference-mission-{name}.json`)
...
**Context**: Determine how comprehensive E2E tests should be for Reference Mission.
...
**Consequences**:
- Reference Mission available for E2E testing immediately
...
```javascript
// tests/e2e/reference-mission.spec.js
describe('Reference Mission Mission', () => {
  beforeEach(async () => {
...
- Reference Mission is configuration only (no new code)
...
- README warns that Reference Mission is for demo/testing only
...
- Mission directory structure created:
  ...
  - `Missions/Reference-Mission/Layers/Vectors/`
  - `Missions/Reference-Mission/Layers/Legends/`
  - `Missions/Reference-Mission/Layers/Tiles/` (with README)
  - `Missions/Reference-Mission/Layers/Images/` (with README)
  - `Missions/Reference-Mission/Layers/Models/` (with README)
  - `Missions/Reference-Mission/Data/` (with README)
- **`config.reference-mission.json`** with:
  - Mission metadata (msv: mission="Reference-Mission", Earth region)
  - Projection: Web Mercator (EPSG:3857)
...
- **Placeholder raster layers** (TODOs, no blocking):
  - Local tile layer: `"url": "Missions/Reference-Mission/Layers/Tiles/basemap/{z}/{x}/{y}.png"` (404 okay)
  - Image layers: `"url": "Missions/Reference-Mission/Data/sample.tif"` (404 okay)
  - Data layer: `"url": "Missions/Reference-Mission/Data/elevation.tif"` (404 okay)
...
- **README.md complete** with:
  - Overview: Purpose of Reference Mission
...
- **Playwright smoke tests**:
  - Single file: `tests/e2e/reference-mission.spec.js`
...
- **AGENTS.md updated** with Reference Mission reference
- **Constitution compliance** verified

**Success Criteria**:
- README is comprehensive and clear
- Admins understand how to use Reference Mission
...
**Risk 4**: Reference Mission becomes stale as MMGIS adds features
...
- **Mitigation**:
  ...
  - Pre-merge checklist includes Reference Mission update ✅
  - Version Reference Mission to match MMGIS version ✅
  - CI/CD validates Reference Mission config loads ✅
...
**Risk 6**: Confusion between Reference Mission and production missions
...
- **Mitigation**:
  ...
  - Mission name "Reference Mission" is obviously non-production
