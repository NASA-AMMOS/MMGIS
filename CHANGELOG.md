...
- **Breaking (Deployments): titiler-pgstac env vars renamed.** `POSTGRES_USER/PASS/HOST/PORT/DBNAME` → `PGUSER/PGPASSWORD/PGHOST/PGPORT/PGDATABASE` for the `titiler-pgstac` service (Compose and `adjacent-servers/titiler-pgstac/.env.example`). The `stac` and `tipg` services keep `POSTGRES_*`.
