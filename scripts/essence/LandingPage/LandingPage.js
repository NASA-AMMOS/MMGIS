define([
    ...
    var LandingPage = {
        init: function(missions, forceError) {
            if (forceError) {
                makeMissionNotFoundDiv()
                return
            }

            var missionUrl = QueryURL.checkIfMission()

            //If there's only one mission, go straight to it
            if (missions.length == 1) missionUrl = missions[0]
            ...
            if (missionUrl == false) {
                ...
                $('#configIcon').on('click', function() {
                    ...
                    else window.location.href = window.location.href + 'config'
                })
                //Attributions
