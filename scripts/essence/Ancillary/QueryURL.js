    ...
    */
        writeCoordinateURL: function(
            ...
            for (var l in L_.toggledArray) {
                if (
                    L_.toggledArray[l] &&
                    L_.layersDataByName[l].type !== 'header'
                )
                    layersOnString +=
                        l +
                        '$' +
                        parseFloat(L_.opacityArray[l]).toFixed(2) +
                        ','
            }
            layersOnString = layersOnString.substring(
                0,
                layersOnString.length - 1
            )
            if (layersOnString.length > 0)
                urlAppendage += '&on=' + layersOnString
