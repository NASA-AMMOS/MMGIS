...
define(['jquery', 'd3', 'Layers_', 'ToolController_', 'semantic'], function(
    $,
    d3,
    L_,
    ToolController_
    ...
    // prettier-ignore
    var modalFormSignIn =
  "<div style='font-family: venus; font-size: 40px; text-align: center;'>MMGIS</div>" +
        ...
        init: function() {
            ...
            $('#loginForm').submit(function(e) {
                ...
                if (!Login.signUp) {
                    if (validate.username && validate.password) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: 'scripts/essence/Ancillary/Login/login.php',
                            data: values,
                        ...
                        // This ajax logs in the user in the API service
                        $.ajax({
                            ...
                            success: function(user) {
                                if (user.id) {
                                    ...
                                    // Now we need to store these variables in a cookie session
                                    document.cookie = 'userCookie=userData'
                                    document.cookie =
                                        'userCookie=' +
                                        JSON.stringify({
                                            id: userId,
                    ...
                    if (Login.loggedIn) {
                        ...
                        if (Login.beganLoggedIn) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url:
                                    'scripts/essence/Ancillary/Login/logout.php',
                            })
                        }
                        ...
                        d3.select('#loginUser')
                            ...
                            .html('')
                        d3.select('#loginoutButton').attr('title', 'Login')
                        d3.select('#loginoutButtonIcon').attr(
                            'class',
                            'mdi mdi-login-variant mdi-18px'
                        )
                        /*
            ...
            */
                ...
                .attr(
                    'class',
                    Login.loggedIn
                        ? 'mdi mdi-logout-variant mdi-18px'
                        : 'mdi mdi-login-variant mdi-18px'
                )
