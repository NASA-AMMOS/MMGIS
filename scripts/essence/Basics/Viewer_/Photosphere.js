define(['d3', 'Formulae_', 'Layers_', 'Sprites', 'Hammer', 'WebVR'], function(
    d3,
    F_,
    L_,
    Sprites,
    ...
    THREE.Photosphere = function(domEl, lookupPath, options) {
        ...
        function init() {
            ...
            if (webglSupport) {
                ...
                orientationControls = new THREE.DeviceOrientationControls(
                    camera
                )
                ...
                spacedPoints.vertices.forEach(function(vertex) {
                    // fill the array
                    if (vertex.y < 0)
                        vertexColors.push(new THREE.Color(0xffff00))
        ...
        function resetLayerMaterials() {
            for (var l in layers) {
                for (var p in layers[l]) {
                    layers[l][p].point.material.dispose()
                    layers[l][p].point.material =
                        layers[l][p].point.savedMaterial
        ...
        function makeModifiedTexture(image, az, el, dim, callback) {
            ...
            var downsizing = isMobile.iPhone()
        ...
        function buildAzElIndicators() {
            ...
            azIndicator.style.position = 'absolute'
            azIndicator.style.top = '0'
            azIndicator.style.left = '50%'
            azIndicator.style.transform = 'translateX(-50%)'
            azIndicator.style.background = 'rgba(0,0,0,0.7)'
            azIndicator.style.padding = '0px 6px'
            azIndicator.style.zIndex = 1010
            azIndicator.style.cursor = 'pointer'
            domEl.appendChild(azIndicator)

            elIndicator = document.createElement('div')
            elIndicator.style.position = 'absolute'
            elIndicator.style.top = '50%'
            elIndicator.style.left = '0'
            elIndicator.style.transform = 'translateY(-50%)'
            elIndicator.style.background = 'rgba(0,0,0,0.7)'
            elIndicator.style.padding = '0px 6px'
