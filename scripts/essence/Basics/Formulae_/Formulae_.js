...
define(['turf', 'fileSaver'], function (turf) {
    ...
    var Formulae_ = {
                        ...
                        for (var j = 0; j < coords[i].length; j++) {
                            if (coords[i][j][0].constructor === Array) {
                                for (var k = 0; k < coords[i][j].length; k++) {
                                    if (
                                        coords[i][j][k][0].constructor === Array
                                    ) {
                                        for (
                                            var l = 0;
        ...
        sortGeoJSONFeatures(geojson) {
            ...
            else if (geojson.hasOwnProperty('Features'))
        ...
        geoJSONArea(g) {
            ...
            function ringArea(coords) {
                ...
                if (coordsLength > 2) {
                    ...
                    area =
                        (area * Math.pow(Formulae_.radiusOfPlanetMajor, 2)) / 2
                }
        ...
        tileIsContained(xyzContainer, xyzContained, useLast) {
            var contains = this.tileContains(
                xyzContainer,
                xyzContained[2],
                useLast
            )
                ...
                if (layer._latlngs[i].lat != null) {
                    ...
                    for (let j = 0; j < layer._latlngs[i].length; j++) {
                        if (layer._latlngs[i][j].lat == null)
                            return (
                                Formulae_.getFeatureLatLngs(layer.feature) || []
                            )
                        let ll = {
...
/*
const isScreenRetina = (devicePixelRatio =
    (window.devicePixelRatio ||
        window.webkitDevicePixelRatio ||
        window.mozDevicePixelRatio ||
        1) > 1.6 && navigator.userAgent.toLowerCase().indexOf('mac os') != -1)
*/
