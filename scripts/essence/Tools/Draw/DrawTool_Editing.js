define([
    ...
    var Editing = {
        ...
        showContextMenu: function(
            ...
            if (!deselecting) {
                ...
                style.weight = style.weight || '4'
                style.fillColor = style.fillColor || 'black'
                style.fillOpacity = style.fillOpacity || '0.2'
                ...
                if (DrawTool.contextMenuLayers.length > 0) {
                    DrawTool.contextMenuLayer =
                        DrawTool.contextMenuLayers[0].layer
                    if (
                        typeof DrawTool.contextMenuLayer.toGeoJSON ===
                        'function'
                    )
                        DrawTool.contextMenuLayerOriginalGeoJSON = DrawTool.contextMenuLayer.toGeoJSON()
                    else
            ...
            DrawTool.contextMenuLayer.resetGeoJSON = function() {
                if (
                    typeof DrawTool.contextMenuLayer.editEnabled === 'function'
                ) {
                    var reenableEdit = false
                    if (
                        typeof DrawTool.contextMenuLayer.editEnabled ===
                        'function'
                    )
                    ...
                    if (
                        reenableEdit &&
                        typeof DrawTool.contextMenuLayer.enableEdit ===
                            'function'
                    )
    ...
    "<div class='drawToolContextMenu'>",
        ...
        "</div>"
            ...
            function resetShape(justThis) {
                for (var c in DrawTool.contextMenuLayers) {
                    ...
                    if (
                        ...
                        updateFillOpacity(
                            l.properties.style.fillOpacity,
                            l.shape
                        )
            ...
            $('.drawToolContextMenu .styleprop .picker').on(
                'click',
                function() {
                ...
                if (!layer || (layer && typeof layer.setStyle === 'function')) {
                    ...
                    for (var c in DrawTool.contextMenuLayers) {
                        if (
                            ...
                            $(
                                '#DrawToolAnnotation_' + p.file_id + '_' + p.id
                            ).css('color', resetStyle.fillColor)
                        }
                    }
                }
            ...
            //DESCRIPTION
            $('#drawToolContextMenuPropertiesDescription').on(
                'input',
                function() {
                    var v = $(this).val()
            ...
            function setProperties(newProperties) {
                ...
                if (
                    hasWidth &&
                    (force || DrawTool.contextMenuChanges.style.width)
                )
                ...
                if (
                    ...
                    newProperties.style.lineCap = $(
                        '.drawToolContextMenu .linecap'
                    )
                        .attr('v')
                        .toLowerCase()
                if (
        ...
        cmLayerDragOn: function() {
            if (
                ...
                DrawTool.contextMenuLayer.feature.hasOwnProperty(
                    'properties'
                ) &&
                DrawTool.contextMenuLayer.feature.properties.annotation == true
            ) {
                ...
                DrawTool.contextMenuLayer.on('mousedown', DrawTool.cmLayerDown)
