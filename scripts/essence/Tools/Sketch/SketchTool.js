    ...
    */
    ...
    function interfaceWithMMWebGIS(domId) {
        ...
        $('.mmgisRadioBarVertical#sketchToolShape div').click(function() {
            $('.mmgisRadioBarVertical#sketchToolShape div').removeClass(
                'active'
            )
        ...
        $('.mmgisRadioBarVertical#sketchToolBehindInFront div').click(
            function() {
                $(
                    '.mmgisRadioBarVertical#sketchToolBehindInFront div'
                ).removeClass('active')
                $(this).addClass('active')
            }
        )
        ...
        optionValue = $('#sketchTool #sketchToolBehindInFront .active').attr(
            'id'
        )
        if (optionValue == 'sketchToolBehind') SketchTool.speState = 'under'
        else if (optionValue == 'sketchToolThrough')
        ...
        $('#sketchToolEditColor').click(function() {
            if (!colorTrans) {
                ...
                if (colorOpen) {
                                    ...
                                    $('#sketchToolEditColors_' + i).click(
                                        (function(i) {
                                            return function() {
                                                ...
                                                $('#sketchToolEditName').val(
                                                    SketchTool
                                                        .drawVarColorLegend[i]
                                                        .name
                                                )
                                                $(
                                                    '#sketchToolEditDescription'
                                                ).val(
                                                    SketchTool
                                                        .drawVarColorLegend[i]
                                                        .value
                    ...
                    colorTrans = true
                    $('#sketchToolEditColors').fadeOut(
                        400,
                        'swing',
                        function() {
                            $('#sketchToolEditColors').remove()
                            $('#sketchToolEditName').fadeIn(
                                400,
                                'swing',
                                function() {
                                    colorTrans = false
        ...
        function _dt0() {
            if (SketchTool.draw_erase_edit_delete != 'draw') {
                ...
                setTimeout(function() {
                    $('#sketchTool #sketchToolDrawPanel').css(
                        'display',
                        'block'
                    )
                }, 400)
                ...
                $('#sketchTool #sketchToolBehindInFront').removeClass(
                    'disabled'
                )
        ...
        function _dt11() {
            if (SketchTool.draw_erase_edit_delete != 'edit') {
                ...
                $('#sketchTool #sketchToolEditPanel').css('display', 'none')
                $('#sketchTool #sketchToolDrawPanel').css('display', 'none')
                $('#sketchTool #sketchToolEditSpecifics').css(
                    'display',
                    'inherit'
                )
        ...
        function _dt3() {
            ...
            //update input name
            $('#sketchTool #sketchToolInputName').val(
                $(this)
                    .find('p')
                    .text()
            )
    ...
    function reloadSPEPolys(geojsonFile, noPointerEvents, useOpacity) {
        ...
        if (isDB) {
            ...
            $.getJSON(geojsonFile, { _: new Date().getTime() }, function(data) {
                if (
                    Map_.map.hasLayer(L_.layersGroup[SketchTool.siteDrawings])
                ) {
                    Map_.map.removeLayer(
                        L_.layersGroup[SketchTool.siteDrawings]
                    )
                }
                //sort data so point features are always on top of lines and lines always on top of polygons
                F_.sortGeoJSONFeatures(data)
                ...
                if (L_.toggledArray[SketchTool.siteDrawings] == true) {
                    ...
                    if (!SketchTool.removed) {
                        ...
                        Globe_.addVectorTileLayer({
                            ...
                            layers:
                                L_.layersGroup[SketchTool.siteDrawings]._layers,
                        })
                    }
                }
            })
    ...
    function editSave(feature) {
        ...
        var newColor = F_.rgb2hex(
            $('#sketchToolEditColor').css('background-color')
        )
