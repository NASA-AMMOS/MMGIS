<?php
  ...
  if (php_sapi_name() == 'cli') {
    ...
    echo makeMission( $argv[1], $argv[2] );
  }
  else {
    echo makeMission( $_POST['missionname'], $_POST['password'] );
  }
  umask( $old );
  ...
  function makeMission( $missionname, $password ) {
    ...
    if( !in_array( strtolower( $missionname ), $currentMissionFolders ) ) {
      ...
      if( $status == 'success' ) {
        $aCCMN = addConfigconfigJSONMissionName( '..', $missionname );
        if( !$aCCMN ) {
          $status = 'failure';
  ...
  function cpMissionTemplate( $to, $named ) {
    ...
    if( !mkdir( $to . '/' . $named, 0777 ) ) return false;
      ...
      if( !mkdir( $to . '/' . $named . '/Database', 0777 ) ) return false;
        //mmgis.sqlite3
        if( !copy( '../resources/MissionTemplate/Database/mmgis.sqlite3', $to . '/' . $named . '/Database/mmgis.sqlite3' ) ) return false;
        if( !chmod( $to . '/' . $named . '/Database/mmgis.sqlite3', 0666 ) ) return false;
      //Drawn
      if( !mkdir( $to . '/' . $named . '/Drawn', 0777 ) ) return false;
        //_speDrawings.geojson
        if( !copy( '../resources/MissionTemplate/Drawn/_speDrawings.geojson', $to . '/' . $named . '/Drawn/_speDrawings.geojson' ) ) return false;
        if( !chmod( $to . '/' . $named . '/Drawn/_speDrawings.geojson', 0666 ) ) return false;
      //Layers
      if( !mkdir( $to . '/' . $named . '/Layers', 0777 ) ) return false;
      //config.json
      if( !copy( '../resources/MissionTemplate/config.json', $to . '/' . $named . '/config.json' ) ) return false;
