...
/**
 * Kitchen Sink Demo Mission - Smoke Tests
 *
 * These tests validate the Kitchen Sink demo mission loads correctly and
 * all core features are present. They run with FORCE_CONFIG_PATH set to
 * Missions/Kitchen-Sink/config.kitchen-sink.json.
 *
...
test.describe('Kitchen Sink Demo Mission - Smoke Tests', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Navigate to Kitchen Sink mission
    await page.goto('/');
  });
  ...
  test('mission loads without crashes', async ({ page }) => {
    ...
    // Check page title
    await expect(page).toHaveTitle(/MMGIS Kitchen Sink Demo|MMGIS/i);
  ...
  test('all configured tools are present', async ({ page }) => {
    ...
    // Expected tools from Kitchen Sink configuration
    const expectedTools = [
      'Identifier',
      'Layers',
      'Legend',
  ...
  test('layers tool lists all vector layers', async ({ page }) => {
    ...
    // Expected vector layer names from Kitchen Sink config
