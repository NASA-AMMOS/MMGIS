...
test.describe("computeWindowResize", () => {
  test("scales panels proportionally", () => {
    ...
    expect(result.pxIsViewer + result.pxIsMap + result.pxIsGlobe).toBeCloseTo(
      1200,
      0,
    );
  });
 ...
 * in the map, globe and viewer singletons), so this asserts at the source
 * level that the sync isn't limited to the globe's first open.
 */
