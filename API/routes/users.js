        ...
        User.create(newUser)
          .then(created => {
            ...
            User.update(
              ...
              .then(() => {
                logger.info(req.body.username + " signed up and logged in.");
                res.send({
                  status: "success",
                  username: created.username,
                  token: req.session.token,
        ...
        function pass(err, result, again) {
          if (result) {
            ...
            User.update(
              ...
              .then(() => {
                logger.info(
                  user.username + " " + (again ? "re" : "") + "logged in."
                );
            ...
            logger.info("Invalid password provided for user: " + user.username);
            res.send({
              status: "failure",
...
router.post("/logout", function(req, res) {
  ...
  if (MMGISUser == false) {
    ...
    User.update(
      ...
      .then(() => {
        logger.info(MMGISUser.username + " logged out.");
