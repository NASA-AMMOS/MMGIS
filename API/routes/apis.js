...
router.get('/layer=:layer_name', function( req, res, next){
    ...
    let query_1 = "SELECT 'SELECT ' || array_to_string(ARRAY("
    + "SELECT 'S' || '.' || c.column_name FROM information_schema.columns As c WHERE table_name = '" 
    + layerName + "' AND  c.column_name NOT IN('geom') ), ',') || ' FROM " + layerName + " As S' As sqlstmt;"

    db.any( query_1 ).then( function( d1 ) {
        // Check the result
        console.log( d1 );
        let queryTOexecute = d1[0].sqlstmt;
        let query_2 = " SELECT 'FeatureCollection' As type, array_to_json(array_agg(f))" + 
        " As features FROM ( SELECT 'Feature' As type, ST_AsGeoJSON(sec1.geom)::json As geometry, " 
        + "row_to_json(sec2) As properties FROM " 
        + layerName + " AS sec1 INNER JOIN (" 
        + queryTOexecute  + ") AS sec2 ON sec1.id = sec2.id) As f;";

        // console.log("Second Query: ", query_2)

        db.any( query_2 ).then( function( d2 ){
            res.send(d2[0]);
        }).catch(function (err) {
...
router.get( '/all_targets_elev/', function( req, res, next){
    ...
    let query = "SELECT ST_Z(geom) AS elevation FROM targets ORDER BY elevation ASC;";
    ...
    db.any( query ).then( function( result ){
        ...
        // This part is to check the parameters 
        // logger.info( "==============================================================" );
        // logger.info( "Min elevation: " + minElev + ", Max elevation: " + maxElev);
        // logger.info( "Num of Bins: " + numOfBins );
        // logger.info( "Data length: " + data.length );
        // logger.info( "Difference between max and min: " + diffBetweenMinMax );
        // logger.info( "Bin size: " + diffBetweenMinMax/numOfBins );
        // logger.info( "==============================================================" );
        ...
        while( d <= data.length ){
            ...
            if( currElev >= lowerElev + binSize){
                ...
                while(currElev >= lowerElev + binSize){
                    ...
                    dataPoints.push( { Bin: bin, NumOfPoints: bin.length, BinIndex: binCount } );
...
router.get('/all_targets', function(req, res, next){
    let query = "SELECT row_to_json(fc) FROM (SELECT 'FeatureCollection' As type, "+
 ...
 */
router.get('/uid=:user_id/tkn=:token/all_targets_name', checkAuthentication, function(req, res, next){
 ...
 */
router.get('/uid=:user_id/tkn=:token/bbox=:bbox', checkAuthentication, function(req, res, next) {
    ...
    'FROM public.\"targets\" WHERE \"targets\".geom && ST_MakeEnvelope(' + bbox + 
 ...
 */
router.get('/uid=:user_id/tkn=:token/bbox=:bbox/inst=:instrument&mat=:material&op=:operand&amont=:amount', checkAuthentication, function(req, res, next) {
    
    // Available materials in the data_ccam table
    let chemcam_materials = ['sio2', 'tio2', 'al2o3', 'feot', 'mgo', 'cao', 'na2o', 'k2o'];
    let apxs_materials = ['na2o', 'mgo', 'al2o3', 'sio2', 'p2o5', 'so3', 'cl', 'k2o', 'cao', 'tio2', 'cr2o3', 'mno', 'feo', 'ni', 'zn', 'br'];
    ...
    switch( requestedItems.Instrument ){
        case "ccam":
            if(!chemcam_materials.includes(requestedItems.Material)){
                res.send({"Material": requestedItems.Material, "Message": "Error: The material is not avalable in our records!"});
            }else{
                ...
                " FROM targets As tar1 INNER JOIN (SELECT tar.target_plan, tar.sol, tar.rmc, tar.image_id, chc."+ requestedItems.Material +
                " FROM targets AS tar, data_" + requestedItems.Instrument + " AS chc WHERE tar.target_plan = chc.target_plan AND "+ requestedItems.Material + " " + requestedItems.Operand + " " + requestedItems.Amount + 
                " GROUP BY tar.target_plan, chc." + requestedItems.Material + ") As tar2 ON tar1.target_plan = tar2.target_plan ) As f)  As fc;";
            }
            break;
        case "apxs":
            if(!apxs_materials.includes(requestedItems.Material)){
                ...
                " FROM ( SELECT 'Feature' As type, ST_AsGeoJSON(tar1.geom)::json As geometry, row_to_json(tar2) As properties" +
                " FROM targets As tar1 INNER JOIN (SELECT tar.target_plan, tar.sol, tar.rmc, tar.image_id, chc."+ requestedItems.Material +
                " FROM targets AS tar, data_chemcam AS chc WHERE tar.target_plan = chc.target_plan AND "+ requestedItems.Material + " " + requestedItems.Operand + " " + requestedItems.Amount + 
                " GROUP BY tar.target_plan, chc." + requestedItems.Material + ") As tar2 ON tar1.target_plan = tar2.target_plan ) As f)  As fc;";
            }
            break;
        default:
            res.send({"Instrument": requestedItems.Instrument, "Message": "Error: Data is not avalable for this instrument!"});
    }
...
router.get('/inst=:instrument/', function(req, res, next){
	...
	// Get the data from database
    db.any(query, 'data_' + instrument).then( function (data) {

        if( data.length >= 1 ){
            res.send(data);
        } 
        if (data.length === 0) {
            res.send({result: "There is no data for table called data_" + instrument + "!"});
        }
    }).catch( function (err) {
        logger.error("Error in api.js: " + err);
        return next(err);
 ...
 */
router.get('/inst=:instrument/data_min_avg_max&mat=:material', function(req, res, next){
	...
	// let query = "SELECT " + material + " FROM data_" + instrument + " ORDER BY " + material + " ASC;";
	// let query = "SELECT min(" + material + ") AS \"min\", avg(" + material + ") AS \"avg\", max(" + material + ") AS \"max\" FROM data_" + instrument + ";";
	logger.info("\nQuery: " + query + "\n");

    // Get the data from database
    db.any(query).then(function (d) {
    	...
    	if (l % 2 === 1){
            ...
            median =  (a % 1) >= 0.5 ? data[Math.ceil(l/2)] : data[Math.floor(l/2)];
 ...
 */
router.get('/uid=:user_id/tkn=:token/get_query_names/', checkAuthentication, function(req, res, next){
    let uID = req.params.user_id;
    let allQueryNames_query = "SELECT id, name, description FROM user_queries WHERE user_id=$1;"

    // Get the data from database
    db.any( allQueryNames_query, [uID] ).then( function (data) {
        res.send( data );
    }).catch( function (err) {
        logger.error("Error in api.js: " + err);
        return next(err);
 ...
 */
router.get('/uid=:user_id/tkn=:token/get_query/qid=:query_id', checkAuthentication, function(req, res, next){
    ...
    db.any( getQueryRecord, [queryID] ).then( function (data) {
        res.send( data[0] );
    }).catch( function (err) {
        logger.error("Error in api.js: " + err);
        return next(err);
 ...
 */
router.post('/uid=:user_id/tkn=:token/run_loaded_query', checkAuthentication, function(req, res, next){
    // Get the query string from html
    let loadedQuery = req.body.query;
    
    // Get selected query data from database
    db.any( loadedQuery ).then( function (data) {
...
router.get('/uid=:user_id/tkn=:token/rmq/id=:query_id', checkAuthentication, function(req, res, next){
    let queryID = req.params.query_id;
    let deleteQuery = "DELETE FROM user_queries WHERE id=$1;"

    // Get selected query data from database
    db.any( deleteQuery, [queryID] ).then( function (data) {
 ...
 */
router.post('/uid=:user_id/tkn=:token/final_query_submit/op=:operation/q_name=:name/', checkAuthentication, runFinalQuery);
...
async function runFinalQuery(req, res, next){
    ...
    for( i = 0; i < components.length; i++ ){
        ...
        if( compType === 'N' || compType === 'S' ){
            ...
            if ( compType === 'N' ){
                ...
                numStrPartQuery += "(" + instrument + "." + fieldName + " >= " + inputRange[0] + 
                " AND " + instrument + "." + fieldName + " <= " + inputRange[1] + ") "; // + component.and_or + " ";    
            }
            ...
            if ( compType === 'S' ){
                ...
                if ( isnum ){
                    ...
                    numStrPartQuery += "(" + fieldName + " LIKE '%" + valueForField + "%') ";
                }
            }
        ...
        if( compType === 'G' ){
            ...
            if ( shape === 'line_buffer' || shape === 'circle_buffer' ){      // Create line buffer or circle buffer 
        ...
        if( i < components.length - 1 ){
            ...
            if( (components[i+1].comp_type === 'N' || components[i+1].comp_type === 'S') && !firtsNumStrComp ){
    ...
    baseQuery = "SELECT row_to_json(fc) FROM ( " + 
                ...
                "SELECT tar_2.target_plan, " + selectedColumns + " FROM targets AS tar_2, ( " + 
                "SELECT DISTINCT ON (target_plan) * FROM data_ccam AS ccam " + numStrPartQuery + 
                " ) AS sec_0 WHERE tar_2.target_plan = sec_0.target_plan" + geometryPartQuery +
                " ) AS sec_1 ON tar_0.target_plan = sec_1.target_plan" + ") AS f) As fc;";
    ...
    if( operation === 'save' ){
        ...
        let saveQuery = "INSERT INTO user_queries(user_id, name, description, query, executed_on) values ($1, $2, $3, $4, $5);"
        ...
        db.any( saveQuery, [req.params.user_id, queryName, queryDescription, baseQuery, curntTime, ])
          .then( function(r){
              res.send(true);
              logger.info( "Inserting into user_queries table was successful!" );
        }).catch(function (err) {
            logger.error("Error in api.js: " + err);
            return next(err);
        }); 
