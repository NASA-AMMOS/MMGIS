...
- **Visibility Sampling rate** — A `1x…32x` dropdown in the charts panel header (right of the Horizon Polygon checkbox) sets how many samples are computed per sweep timestep. `1x` = one ray per timestep; higher rates add interpolated samples between sweep frames for a smoother timeline. The request's `stepSeconds` is `sweepStep ÷ samplingRate`.
